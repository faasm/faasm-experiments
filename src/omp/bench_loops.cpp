...
constexpr auto DOCKER_CMD =
  "/usr/bin/docker run --cpus=20 -v /usr/local/code/faasm/wasm:/wasm --rm "
  "faasm/omp-musl:0.1.5 docker_benchmarker ";

void nativeRun(const std::string& funcName, int numRuns);

void wasmRun(const std::string& user,
             const std::string& funcName,
             int wasmIterations,
             const std::string& outfile);
void wasmRunIteration(std::ofstream& profOut,
...
int main(int argc, char* argv[])
{
    ...
    std::string outfile =
      std::string("/usr/local/code/faasm/wasm/omp/") + funcName + "/bench.csv";
    // Clean up for container (and ensuring we will be writing to the same file
    boost::filesystem::remove(outfile);

    logger->info("Benchmarking {} ({}x native and {}x wasm)",
                 funcName,
                 nativeIterations,
                 wasmIterations);
...
void nativeRun(const std::string& funcName, int numRuns)
{
    ...
    const std::string execCmd =
      fmt::format("{} {} {}", DOCKER_CMD, funcName, numRuns);
             ...
             const std::string& outfile)
{
    ...
    if (!boost::filesystem::exists(outfile)) {
        throw std::runtime_error("Could not find native benchmark output at " +
                                 outfile);
    }
    ...
    for (int run = 1; run <= wasmIterations; run++) {
        ...
        for (size_t i = 0; i < iterations.size(); i++) {
            wasmRunIteration(
              profOut, call, iterations[i], iter_names[i], 1, cachedModule);
            for (int num_threads = 2; num_threads < 25; num_threads += 2) {
                wasmRunIteration(profOut,
                                 call,
                                 iterations[i],
                                 iter_names[i],
                                 num_threads,
                      ...
                      wasm::WAVMWasmModule cachedModule)
{
    ...
    profOut << iteration_name << num_threads << ",wasm," << wasmIterationTime
            << std::endl;
}
