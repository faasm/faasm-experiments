...
#define N(x, y)                                                                \
    (eassert(0 <= x && x < size && 0 <= y && y < size),                        \
     (REALLY_VERBOSE ? fprintf(VERBOSE_OUT,                                    \
                               "i, j, k = %d, %d, %d: reading N[%d][%d]\n",    \
                               i,                                              \
                               j,                                              \
                               k,                                              \
                               x,                                              \
...
int getint(char* prompt)
{
    ...
    int result;
    int i = 0;
    while (fprintf(stderr, "%s%s", prompt, terminate),
           result = scanf("%d", &i),
           result != 1 && result != EOF) {
...
#if !defined size
    int size =
      getint("Enter length of random mRNA sequence (2200 is average for human "
             "mRNA): "); // Average (human) mRNA length is 2200; there's one
                         // that's 5000, though
#endif
...
#pragma scop
    for (i = size - 1; i >= 0; i--) {
        for (j = i + 1; j < size; j++) {
            ...
            {
                int k; /* local k to allow N macro to look at k and get -1 above
                          and real k here */
